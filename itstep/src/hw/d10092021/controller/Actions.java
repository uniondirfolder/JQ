...
public class Actions implements Action {
    ...
    public void createTable() {
        try {
            ...
            statement.executeUpdate(cmd.create(DB.SQLITE, SQLite.TABLE, new Task(0, 0)));
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            disconnect();
        ...
        try {
            addTask(new Task(new Date().getTime(), "Готовиться к собесу по Java", 1));
    ...
    public void updateTask(Task task) {
        try {
            ...
            preparedStatement = connection.prepareStatement(cmd.update(DB.SQLITE, SQLite.RECORD, task));
            preparedStatement.setLong(1, task.getCreatedDate());
            preparedStatement.setString(2, task.getDescription());
            preparedStatement.setLong(3, task.getGroup_id());
            connection.commit();
    ...
    public List<Task> getTasks() {
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery(cmd.read(DB.SQLITE, SQLite.TABLE, new Task(0, 0)));
            if (rs != null) {

                while (rs.next()) {
                    list.add(new Task(rs.getLong(1), rs.getString(2), rs.getLong(3)));
                }
                return list;
            }
        } catch (Exception e) {
